...
def get_user_header(i):
    header = bytes()
    loginDataType = 0
    h1 = FRAME_TYPE.to_bytes(length=1, byteorder='little', signed=True)
    h2 = loginDataType.to_bytes(length=1, byteorder='little', signed=True)
...
def get_audio_header(size):
    h1 = FRAME_TYPE.to_bytes(length=1, byteorder='little', signed=True)
    h2 = AUDIO_TYPE.to_bytes(length=1, byteorder='little', signed=True)
...
def get_video_header(size):
    h1 = FRAME_TYPE.to_bytes(length=1, byteorder='little', signed=True)
    h2 = VIDEO_TYPE.to_bytes(length=1, byteorder='little', signed=True)
    sizes = size.to_bytes(length=4, byteorder="little", signed=False)
    return (h1 + h2 + sizes)
